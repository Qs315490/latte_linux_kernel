							...
							enum ktd2026_led_mode mode, u8 bright)
{
	...
	data = (struct ktd2026_data *)i2c_get_clientdata(b_client);
...
void ktd2026_set_timerslot_control(int timer_slot)
{
	...
	data = i2c_get_clientdata(b_client);
...
void ktd2026_set_trise_tfall(int trise, int tfall, int tscale)
{
	...
	data->shadow_reg[KTD2026_REG_EN_RST] |= tscale << CNT_RISEFALL_TSCALE_SHIFT;
}
...
static int ktd20xx_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	printk("[%s]: Enter!\n", __func__);
	printk("[%s]: id_table: %s\n", __func__, id->name);
	printk("[%s]: acpi_match_table: %s\n", __func__, client->dev.driver->acpi_match_table->id);

	aid = acpi_match_device(client->dev.driver->acpi_match_table, &client->dev);
	printk("[%s]: acpi_match: %s\n", __func__, aid->id);
	// 一直匹配不上，干脆注释了
	// if (!aid)
	// 	return -ENODEV;

	printk("[%s]: acpi_match_device done\n", __func__);
